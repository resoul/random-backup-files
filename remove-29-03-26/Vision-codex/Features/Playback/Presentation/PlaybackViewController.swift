...
final class PlaybackViewController: UIViewController {
    ...
    private func setupQueuePlayer() {
        ...
        if let next = context.nextItem,
           let stream = next.streamURL(preferredQuality: qualityPreferenceRepository.globalPreferredQuality),
    ...
    private func handleCurrentItemChanged() {
        ...
        guard let nextCtx = currentContext.advancedContext(
            preferredQuality: qualityPreferenceRepository.globalPreferredQuality
        ) else { return }
        ...
        if case let .episode(movieId, season, episode, _, _, _, _, _) = nextCtx {
            let saved = playbackStateRepository
    ...
    private func enqueueItemAfter(_ nextItem: NextEpisodeItem) {
        ...
        guard let stream = probe.streamURL(preferredQuality: qualityPreferenceRepository.globalPreferredQuality),
    ...
    private func saveProgressValues(position: Double, duration: Double) {
        ...
        case let .movie(movieId, studio, quality, streamURL):
            playbackStateRepository.saveMovieProgress(
        ...
        case let .episode(movieId, season, episode, _, _, _, _, _):
            playbackStateRepository.saveEpisodeProgress(
