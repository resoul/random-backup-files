...
// MARK: - MovieFocusCoordinatorDelegate

protocol MovieFocusCoordinatorDelegate: AnyObject {
    func movieFocusCoordinator(_ coordinator: MovieFocusCoordinator, didFocusMovie movie: Movie)
    func movieFocusCoordinatorDidLoseFocus(_ coordinator: MovieFocusCoordinator)
}

// MARK: - MovieFocusCoordinator
//
// Адаптирован из Vision/App/Movie/FocusCoordinator.swift
// Переименован чтобы не конфликтовал с Presentation/Coordinators/FocusCoordinator.swift
// (тот отвечает за переключение фокуса tabBar ↔ content, этот — за backdrop + видеопревью)
//
// Отвечает за:
...
final class MovieFocusCoordinator {

    // MARK: - Dependencies

    weak var delegate: MovieFocusCoordinatorDelegate?
    ...
    func handleFocus(on movie: Movie, cellSize: CGSize) {
        ...
        delegate?.movieFocusCoordinator(self, didFocusMovie: movie)
    ...
    func handleFocusLost() {
        ...
        delegate?.movieFocusCoordinatorDidLoseFocus(self)
    }
    ...
    /// Вызывать из viewDidLayoutSubviews
