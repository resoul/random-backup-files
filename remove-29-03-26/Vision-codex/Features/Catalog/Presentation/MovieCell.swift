...
final class MovieCell: UICollectionViewCell {
    ...
    private let playbackStateRepository = AppDIContainer.shared.makePlaybackStateRepository()
    private let watchHistoryActionsRepository = AppDIContainer.shared.makeWatchHistoryActionsRepository()
    ...
    func configure(with movie: Movie) {
        ...
        if !isSeries {
            if let progress = playbackStateRepository.movieProgress(movieId: movie.id),
            ...
            hideProgress()
            let inProgress = watchHistoryActionsRepository.isSeriesInProgress(movieId: movie.id)
