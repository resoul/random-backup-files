...
final class SerieDetailViewController: BaseDetailViewController {
    ...
    private let seriesTranslationsViewModel: SeriesTranslationsViewModel
    private let qualityPreferenceRepository: QualityPreferenceRepository
    private let seriesPlaybackSelectionRepository: SeriesPlaybackSelectionRepository
    ...
    init(
        ...
        seriesTranslationsViewModel: SeriesTranslationsViewModel = AppDIContainer.shared.makeSeriesTranslationsViewModel(),
        qualityPreferenceRepository: QualityPreferenceRepository = AppDIContainer.shared.makeQualityPreferenceRepository(),
        seriesPlaybackSelectionRepository: SeriesPlaybackSelectionRepository = AppDIContainer.shared.makeSeriesPlaybackSelectionRepository()
    ) {
        self.seriesTranslationsViewModel = seriesTranslationsViewModel
        self.qualityPreferenceRepository = qualityPreferenceRepository
        self.seriesPlaybackSelectionRepository = seriesPlaybackSelectionRepository
    ...
    override func viewDidLoad() {
        ...
        qualityButton.configure(quality: qualityPreferenceRepository.globalPreferredQuality ?? "Авто")
    }
...
//            emptyLabel.isHidden = false
    ...
    private func fetchTranslations() {
        seriesTranslationsViewModel.load(movieId: movie.id)
    }

    private func bindViewModel() {
        seriesTranslationsViewModel.onStateDidChange = { [weak self] state in
            self?.applyTranslationsState(state)
        }
    }

    private func applyTranslationsState(_ state: SeriesTranslationsState) {
        if state.isLoading {
            episodeSpinner.startAnimating()
        } else {
            episodeSpinner.stopAnimating()
        }

        translations = state.translations

        let previousStudio = activeTranslation?.studio
        activeTranslation = state.activeTranslation
        if let studio = activeTranslation?.studio {
            studioPicker.configure(studio: studio)
            if previousStudio != studio {
                activeSeasonIndex = 0
                rebuildSeasonTabs()
    ...
    private func showStudioPicker() {
        ...
        picker.onSelect = { [weak self] translation in
            ...
            self.seriesTranslationsViewModel.select(translation: translation)
        }
    ...
    private func rebuildEpisodes(animated: Bool) {
        ...
        let build = {
            ...
            for (i, folder) in season.folder.enumerated() {
                ...
                let watched = self.isEpisodeWatched(
                    movieId: self.movie.id, season: seasonNum, episode: episodeNum)

                let progress = self.episodeProgress(
                    movieId: self.movie.id, season: seasonNum, episode: episodeNum)
                ...
                row.onWatchToggle = { [weak self] in
                    guard let self else { return }
                    let was = self.isEpisodeWatched(
                        movieId: self.movie.id, season: seasonNum, episode: episodeNum)
                    self.setEpisodeWatched(!was,
                        movieId: self.movie.id, season: seasonNum, episode: episodeNum)
                    row.setWatched(!was)
                }
    ...
    func playEpisode(folder: _FilmixPlayerFolder, seasonIndex: Int, episodeIndex: Int) {
        ...
        let preferred = qualityPreferenceRepository.globalPreferredQuality
    ...
    func finishPlay(url: String, quality: String, folder: _FilmixPlayerFolder,
        ...
        seriesPlaybackSelectionRepository.save(
            movieId: movie.id,
            season: seasonIndex,
        ...
        let resumePos  = episodeProgress(movieId: movie.id, season: seasonNum, episode: episodeNum)?
            .positionSeconds ?? 0
