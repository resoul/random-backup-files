...
class BaseDetailViewController: UIViewController {
    ...
    let movie: Movie
    private let loadMovieDetailUseCase: LoadMovieDetailUseCaseProtocol
    private let favoritesStateRepository: FavoritesStateRepository
    private let watchHistoryActionsRepository: WatchHistoryActionsRepository
    ...
    init(
        ...
        favoritesStateRepository: FavoritesStateRepository = AppDIContainer.shared.makeFavoritesStateRepository(),
        watchHistoryActionsRepository: WatchHistoryActionsRepository = AppDIContainer.shared.makeWatchHistoryActionsRepository(),
        playbackStateRepository: PlaybackStateRepository = AppDIContainer.shared.makePlaybackStateRepository()
    ) {
        self.movie = movie
        self.loadMovieDetailUseCase = loadMovieDetailUseCase
        self.favoritesStateRepository = favoritesStateRepository
        self.watchHistoryActionsRepository = watchHistoryActionsRepository
    ...
    private func favoriteButtonTitle() -> String {
        favoritesStateRepository.isFavorite(id: movie.id)
            ? "✓  В избранном"
            : "+  Добавить в избранное"
    }

    @objc private func toggleFavorite() {
        favoritesStateRepository.toggle(movie)
    ...
    func fetchDetail() {
        ...
        Task { [weak self] in
            ...
            do {
                let detail = try await loadMovieDetailUseCase.execute(path: movie.movieURL)
                self.detailSpinner.stopAnimating()
                self.detail = detail
                self.populateFromDetail(detail)
    ...
    func playMovie(url: String, title: String, studio: String, quality: String) {
        let resumePos = playbackStateRepository.movieProgress(movieId: movie.id)?.positionSeconds ?? 0
        ...
        watchHistoryActionsRepository.touch(movie)
    ...
    func episodeProgress(movieId: Int, season: Int, episode: Int) -> PlaybackProgress? {
        playbackStateRepository.episodeProgress(movieId: movieId, season: season, episode: episode)
    }

    func isEpisodeWatched(movieId: Int, season: Int, episode: Int) -> Bool {
        playbackStateRepository.isEpisodeWatched(movieId: movieId, season: season, episode: episode)
    }

    func setEpisodeWatched(_ watched: Bool, movieId: Int, season: Int, episode: Int) {
        playbackStateRepository.setEpisodeWatched(watched, movieId: movieId, season: season, episode: episode)
    }

    func touchWatchHistory(_ movie: Movie) {
        watchHistoryActionsRepository.touch(movie)
    }
