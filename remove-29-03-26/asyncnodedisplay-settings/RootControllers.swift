...
final class RootControllers: UIViewController {
    ...
    private lazy var searchHintLabel: UILabel = {
        ...
        l.textAlignment = .center
        l.numberOfLines = 2
        l.text = "Введите запрос и нажмите ⏎ для поиска"
        l.isHidden = true
        l.translatesAutoresizingMaskIntoConstraints = false
        return l
    }()
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(searchHintLabel)
        ...
        NSLayoutConstraint.activate([
            ...
            searchHintLabel.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            searchHintLabel.centerYAnchor.constraint(equalTo: view.centerYAnchor),
            searchHintLabel.widthAnchor.constraint(equalTo: view.widthAnchor, multiplier: 0.55),
        ])
    ...
    private func loadFirstPage() {
        if isSearchTab {
            if currentSearchQuery.isEmpty {
                ...
                loadSearchResults(query: currentSearchQuery)
            }
            return
        }
    ...
    private func loadSearchResults(query: String) {
        guard !query.isEmpty else { showSearchHint(); return }
        isFetching = true
        ...
        FilmixService.shared.fetchSearchResults(query: query) { [weak self] result in
            DispatchQueue.main.async {
                ...
                case .failure(let error):
                    self.errorLabel.text = "Ошибка поиска\n\(error.localizedDescription)"
                    self.errorLabel.isHidden  = false
                    self.retryButton.isHidden = false
                }
            }
    ...
    private func loadNextPage() {
        guard !isFetching, !isFavoritesTab, !isWatchHistoryTab, !isSearchTab, let url = nextPageURL else { return }
        isFetching = true
    ...
    private func switchCategory(url: String?,
                                isFavorites: Bool = false,
                                isWatchHistory: Bool = false,
                                isSearch: Bool = false,
                                searchQuery: String = "") {
        let sameState = url == currentCategoryURL
...
extension RootControllers: CategoryTabBarDelegate {
    ...
    func categoryTabBar(_ bar: CategoryTabBar, didSelect category: FilmixCategory) {
        ...
        if category.isSearch {
            switchCategory(url: nil, isSearch: true, searchQuery: "")
    ...
    private func updateTabBarHeight(for category: FilmixCategory) {
        let target: CGFloat
        if category.isSearch {
            target = CategoryTabBar.searchExpandedHeight
        } else if !category.genres.isEmpty {
            target = CategoryTabBar.expandedHeight
        } else {
            target = CategoryTabBar.collapsedHeight
        }
...
extension RootControllers: UICollectionViewDataSource {
...
extension RootControllers: UICollectionViewDelegate {
    ...
    func collectionView(_ cv: UICollectionView, didSelectItemAt ip: IndexPath) {
        ...
        vc.onDismiss = { [weak self] in
            ...
            else if self.isWatchHistoryTab { self.loadWatchHistory() }
        }
...
extension RootControllers: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ cv: UICollectionView,
        ...
        nextPageURL != nil && !isFavoritesTab && !isWatchHistoryTab && !isSearchTab
