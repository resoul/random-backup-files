...
final class StorageSectionView: UIView {
    ...
    private let favoritesStateRepository = AppDIContainer.shared.makeFavoritesStateRepository()
    ...
    private func render(_ info: StorageInfo) {
        ...
        let watchedCount = playbackStateRepository.totalEpisodeCount()
        let favoritesCount = favoritesStateRepository.allCount()
    ...
    private func clearWatched() {
        confirm("Сбросить историю просмотров?\nЭто действие нельзя отменить.") {
            self.playbackStateRepository.clearAllEpisodes()
            self.reload()
        }
    }
